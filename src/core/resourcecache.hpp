...
public:
     ...
     * Name is relative to baseDirectory, the file ending is added
     *automatically.
...
{
    if (!(fs::exists(this->baseDirectory) &&
          fs::is_directory(this->baseDirectory)))
    {
        throw std::runtime_error(
                  "Invalid resource directory: " + baseDirectory.string());
    }
    // createResourceList();
}
template<class T>
...
shared_ptr<T> ResourceCache<T>::get(const std::string& name)
{
    ...
    if (iter == loadedResources.end())
    {
        ...
        if (fileEnding != "")        // Do not add extension if file ending is
                                     // empty
        {
            filename += ("." + fileEnding);
        }
...
// {
// resourceList.clear();
// for (fs::recursive_directory_iterator end, it(resourceDirectory); it != end;
// ++it)
// {
// fs::path item = *it;
