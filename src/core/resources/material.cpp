...
#define MSG_ERROR_PARSING_VALUE "Invalid value in material file"
#define MSG_INVALID_MATERIAL_FILE "Invalid material file"
#define MSG_CANNOT_OPEN_MATERIAL_FILE "Cannot open material file"
                ...
                if (l != 0)
                {
                    ...
                    else if (state == 2)                     // diffuse color
                    {
                        if (l == 5)
                        {
                            ...
                            {
                                // parse values
                                diffuse.r         = boost::lexical_cast<float>(
                                    parts[1]);
                                diffuse.g         = boost::lexical_cast<float>(
                                    parts[2]);
                                diffuse.b         = boost::lexical_cast<float>(
                                    parts[3]);
                                diffuse.intensity = boost::lexical_cast<float>(
                                    parts[4]);
                            }
                            catch (boost::bad_lexical_cast&)
                            {
                                throw InvalidResourceError(
                                          path.string(),
                                          MSG_ERROR_PARSING_VALUE);
                            }
                        }
                        else
                        {
                            throw InvalidResourceError(
                                      path.string(), MSG_ERROR_PARSING_VALUE);
                        }
                    }
                    else if (state == 3)                     // specular color
                    {
                        if (l == 5)
                        {
                            ...
                            {
                                // parse values
                                specular.r         = boost::lexical_cast<float>(
                                    parts[1]);
                                specular.g         = boost::lexical_cast<float>(
                                    parts[2]);
                                specular.b         = boost::lexical_cast<float>(
                                    parts[3]);
                                specular.intensity = boost::lexical_cast<float>(
                                    parts[4]);
                            }
                            catch (boost::bad_lexical_cast&)
                            {
